...
static uint32_t s_sectorSize = 0U; /* Declare variable of sectorSize, "U" means Coding Convention */
...
{
    bool retVal = true; /* Declare a variable to return true or false */
    /* Check sector of Size, if sector divided by 512 is 0 it will assign value for s_sectorSize*/
    if (sectorSize % 512 == 0)
    {
...
{
    size_t numberOfByte = 0; /* Initialize variable of number of byte which received from function fread */
...
{
    size_t numberOfByte = 0; /* To return number of bytes which had been read */
    size_t numberOfByteTemp = 0; /* To count byte number when reading multi sector*/
    ...
    if (fseek(s_image, (s_sectorSize *index), SEEK_SET) == 0 )
    {
        for (i = 0U; i < num; i++)
        {
            offset = (uint64_t)i * s_sectorSize; /* Offset 0, 1, 2, ... * sector size */
...
bool kmc_close_img_file(void)
{
    ...
    if (fclose(s_image) == 0)
    {
        retVal = true;
        s_image = NULL;
        // s_sectorSize = KMC_BYTE_PER_SEC_IN_RDET;
    }
    else 
    {
        retVal = false;
