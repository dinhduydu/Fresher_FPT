...
typedef struct calculate_fatfs_struct
{
	...
	uint32_t FATOffset;					/* Calculate FAT Offset */
	uint32_t ThisFATSecNum;				/* The sector number of the FAT sector that contains the entry for cluster N in the first FAT */
	uint32_t FATSz;						/* Calculate FAT size */
	uint32_t TotSec;					/* Calculate total sector */
	uint8_t FATType;					/* Calculate FAT type */
	uint32_t StartRootDir;				/* Calculate start root directory */
	uint8_t tempBuffer[100]; 			/* Declare static buffer to store value of buffer in data region */
 ...
 * 
 * @param bootInfor Pointer point to bootsector
 * @param headDir header of linked list
 * @param cal pointer point to store calculated variable 
 * @return int8_t 
 */
bool fatfs_init(fatfs_BPB_infor_struct **bootInfor, fatfs_entry_list_struct **headDir, fatfs_calculate_struct **cal);
