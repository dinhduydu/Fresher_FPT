...
void InitStaticLinkList(staticLinkList slList)
{
    ...
    for(i = 0;i < MAX_SIZE_SSL;i++)
    {
        slList[i].next = i+1;
        slList[i].data = 0; // It is used to represent idle nodes when outputting
    }
...
int InsertStaticLinkList(staticLinkList slList,int pos,uint8_t element)
{
	int i =0;
...
int DeleteStaticLinkList(staticLinkList slList,int pos)
{
    ...
    for(i = 1;i < pos-1;i++)//Get the prefix node to delete
    {
        cursor = slList[cursor].next;
    }
